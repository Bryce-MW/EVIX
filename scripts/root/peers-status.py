...
with smtplib.SMTP_SSL("***REMOVED***", 465, context=context) as server:
	...
	for i in sys.stdin:
		...
		if difference == 3 or difference >= 14:
			cursor.execute("SELECT name,contact,provisioned FROM clients INNER JOIN asns ON client_id=id INNER JOIN ips ON ips.asn=asns.asn WHERE ip=%s AND monitor=true", (ip,))
			results = tuple(cursor)
			if len(results) == 0:
				print(f"No result found for {asn} over {ip} (not monitored?)")
				continue
