...
try:
	database = mysql.connector.connect(user='evix', password='***REMOVED***', host='127.0.0.1', database='evix')
...
clients = tuple((i, tuple((ipaddress.ip_address(j['ip']).version, ipaddress.ip_address(j['ip']).packed) for j in next(ips_cursor.execute("SELECT ip FROM ips INNER JOIN asns ON ips.asn=asns.asn WHERE provisioned=true AND client_id=%s ORDER BY (ip not like '%:%') DESC, INET6_ATON(ip)", (i['id'],), multi=True)))) for i in clients_cursor)

for client,_ in sorted(clients, key=lambda x: x[1]):
	asns_cursor.reset()
	connections_cursor.reset()
	asns_cursor.execute("SELECT asn FROM asns WHERE client_id = %s AND EXISTS (SELECT 1 FROM ips WHERE asns.asn=ips.asn AND provisioned=1)", (client['id'],))
	...
	for asn in asns_cursor:
		ips_cursor.reset()
		ips_cursor.execute("SELECT ip,version,provisioned FROM ips WHERE asn = %s ORDER BY (ip not like '%:%') DESC, INET6_ATON(ip)", (asn['asn'],))
