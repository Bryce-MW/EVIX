<?php
	...
	if($asn=='pending')
	{
		...
		if(mysqli_num_rows($res) > 0)
		{
			...
			{
				$tunnquery="SELECT * FROM tunnels WHERE asn=". $row['asn'];
				$tunns=mysqli_query($conn,$tunnquery);

				echo "ASN: ". $row['asn']. "\n";
				echo "Description: ". $row['description']. "\n";
				echo "AS-SET: ". $row['asset']. "\n";
				echo "Contact Email: ". $row['contact']. "\n";
				while($tunnrow=mysqli_fetch_array($tunns))
				{
	...
	if($argv[2])
	{
		if($argv[2]=="ipv6only")
		{
			$ip6_prefix="2602:fed2:fff:ffff:6::";
			for($i=hexdec(10);$i<hexdec('ffff');$i++)
			{
				$query="SELECT * FROM peers WHERE address6='". $ip6_prefix. dechex($i). "' AND ip6only=1";
//				if(mysqli_query($conn,$query)
				$rows=mysqli_num_rows(mysqli_query($conn,$query));
				if($rows==0)
		                {
					$ip6=$ip6_prefix. dechex($i);
					$query="SELECT * FROM additionalips WHERE address6='". $ip6."'";
                                        $rows=mysqli_num_rows(mysqli_query($conn,$query));
                                        if($rows==0)
                                        {
						echo "new user IPv6 is: ". $ip6."\n";
						$query="UPDATE peers SET address6='". $ip6. "',status=2,ip6only=1 WHERE asn=". $asn;
                                                $res=mysqli_query($conn,$query);
                                                if(!$res)
                                                {
