...
outdir='/var/www/evix/static/graphs/'
token='70c81f6873f69c6052da082d223084d0'
...
#curl -H "X-Auth-Token: $token" http://librenms.evix.org/api/v0/portgroups/multiport/bits/11/\?legend=no\&type=multiport_bits\&from=$dayago\&to=$now\&width=$width\&height=$height > daily_tmp.png
curl -H "X-Auth-Token: $token" http://librenms.evix.org/api/v0/devices/72.52.82.6/ports/vmbr0/port_bits > daily_tmp.png
#convert -pointsize 12 -draw "text 300,12 'EVIX Fremont Traffic'" daily_tmp.png daily.png
convert -fill black -pointsize 12 -draw "text 300,12 'EVIX Fremont Traffic'" daily_tmp.png daily.png
#rm daily_tmp.png
mv daily.png $outdir
